...
describe("cell insertion", function()
	...
	it("inserts after a long tilde fence using the outline boundaries", function()
		local result = cells.insert({ "~~~~{r}", "plot(x)", "~~~~", "prose" }, 2, "below", "fenced", "{r}")
		assert.are.same({ "~~~~{r}", "plot(x)", "~~~~", "", "```{r}", "", "```", "", "prose" }, result.lines)
	end)
end)
