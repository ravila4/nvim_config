...
describe("Document outline provider", function()
	...
	it("highlights failed cell markers without duplicating window matches", function()
		...
		local done = vim.tbl_filter(function(match)
			return match.group == "NotebookCellDone"
		end, matches)
		assert.are.equal(1, #done)
		assert.are.equal("✓", vim.fn.matchstr("  ✓ Cell 1: ok", done[1].pattern))
		assert.are.equal("", vim.fn.matchstr("  ✗ Cell 1: 1/0", done[1].pattern))
	end)
