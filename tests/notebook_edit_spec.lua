describe("Notebook structural edits", function()
	...
	it("returns empty records before notebook state is initialized", function()
		local virtuals, records = edit.records(buf)
		assert.are.same({}, virtuals)
	...
	it("shifts saved source locations when inserting a new cell", function()
		edit.executions(buf, { { cell_type = "code", source = "b = 2", execution_count = 1, outputs = {} } })
		edit.insert(buf, 4, { "```python", "", "```", "" })
		assert.are.equal(9, edit.executions(buf, {})[1].start_line)
	end)
