...
local function symbols(buf)
	...
	vim.list_extend(executions, require("config.notebook_edit").executions(buf, saved[path]))
	return require("config.notebook_outline").symbols(vim.api.nvim_buf_get_lines(buf, 0, -1, false), executions, {})
end

function M.request_symbols(callback, opts)
	local buf = vim.api.nvim_get_current_buf()
	local result = symbols(buf)
	sources[vim.api.nvim_get_current_tabpage()] = { buf = buf, win = vim.api.nvim_get_current_win(), symbols = result }
		...
		for tab, source in pairs(sources) do
			...
			elseif vim.api.nvim_win_get_buf(source.win) == source.buf then
				vim.api.nvim_win_call(source.win, function()
					...
					if require("outline").is_open() and not vim.deep_equal(source.symbols, symbols(source.buf)) then
