					...
					on_exit = function(_, code)
						vim.schedule(function()
							if code == 0 then
								...
								vim.notify(
									"[Quick Edit] LM Studio not reachable; claude-only selector",
									vim.log.levels.WARN
								)
							end
							vim.ui.select(items, {
							...
							vim.schedule(function()
								...
								if provider_snapshot == "lmstudio" then
									...
									if
										...
										or not parsed.choices[1]
									then
										vim.notify(
											"[Quick Edit] Invalid JSON: " .. raw:sub(1, 200),
											vim.log.levels.ERROR
										)
										return
									end
									...
									if response == "" then
										vim.notify(
											"[Quick Edit] Empty response (finish="
												.. (parsed.choices[1].finish_reason or "?")
												.. ")",
											vim.log.levels.ERROR
										)
										return
								...
								local result = route_ask and { mode = "ask", text = response }
									or parse_response(response)
