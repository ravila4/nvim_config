			...
			_G.ide_menus = {
				...
				lsp_menu = {
					...
					{ name = " Show Diagnostics", cmd = "lua vim.diagnostic.open_float()", rtxt = "df" },
					{
						...
						cmd = "lua vim.diagnostic.jump({ count = 1, float = true })",
						rtxt = "]d",
					},
					{
				...
				test_menu = {
					...
					{
						name = "  Run All Tests",
						cmd = "lua require('neotest').run.run(vim.fn.getcwd())",
						rtxt = "ta",
					},
					{ name = "separator" },
					...
					{
						name = "  Toggle Watch",
						cmd = "lua require('neotest').watch.toggle(vim.fn.expand('%'))",
						rtxt = "tw",
					},
					{
				...
				if filename:match("%.ipynb$") then
					...
					if has_text then
						table.insert(
							context_menu,
							1,
							{ name = "󰆏 Copy Output", cmd = copy.copy_output_at_cursor, rtxt = "mo" }
						)
					end
					if has_image then
