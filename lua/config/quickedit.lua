						...
						if ok and type(parsed) == "table" and type(parsed.data) == "table" then
							for _, m in ipairs(parsed.data) do
								if type(m.id) == "string" and not m.id:match("^text%-embedding") then
									table.insert(
										items,
										{ label = "lmstudio:" .. m.id, provider = "lmstudio", model = m.id }
									)
								end
							end
						end
...
- Show example inputs and outputs when relevant]]
	...
	function M.quickedit()
		...
		local shorthand_keys = response.shorthand_keys()
		...
		Snacks.input({
			...
			local selected_diagnostics = vim.tbl_filter(function(diagnostic)
				return diagnostic.lnum >= start_line - 1 and diagnostic.lnum <= end_line - 1
			end, all_diagnostics)
			if #selected_diagnostics == 0 then
				selected_diagnostics = all_diagnostics
			end
			instruction = response.expand_instruction(instruction, selected_diagnostics)

			-- Pick system prompt based on whether this is an ask or edit command
			local raw_cmd = instruction_raw:match("^(/[%w_]+)")
			local is_ask = raw_cmd and response.ask_shorthands[raw_cmd]
			...
			local model = M.provider == "claude" and M.claude_model or M.lmstudio_model
			local request = provider.build_request(M.provider, model, prompt)
				...
				on_exit = function(_, exit_code)
					vim.schedule(function()
						...
						if provider_snapshot == "lmstudio" then
							local decoded, decode_error, finish_reason = provider.decode_lmstudio(raw)
							if decode_error == "invalid_json" then
								vim.notify("[Quick Edit] Invalid JSON: " .. raw:sub(1, 200), vim.log.levels.ERROR)
								return
							end
							if decode_error == "empty_response" then
								vim.notify(
									"[Quick Edit] Empty response (finish=" .. (finish_reason or "?") .. ")",
									vim.log.levels.ERROR
								)
								return
							end
							content = decoded.content
							if decoded.degraded then
								degraded_to_ask = not is_ask
								local extra = degraded_to_ask and " (edit → ask)" or ""
								vim.notify(
									"[Quick Edit] Empty content; showing reasoning_content" .. extra,
									vim.log.levels.WARN
								)
							end
						else
						...
						local result = route_ask and { mode = "ask", text = content } or response.parse(content)
			...
			if job_id > 0 and request.stdin then
				vim.fn.chansend(job_id, request.stdin)
		...
		vim.api.nvim_create_autocmd({ "TextChangedI", "TextChanged" }, {
			...
			callback = function()
				...
				if cmd and response.is_shorthand(cmd) then
			...
			vim.fn.jobstart({
				...
				on_exit = function(_, code)
					...
					if code == 7 then
						vim.schedule(function()
							vim.notify(
								"[Quick Edit] Warmup failed — LM Studio server unreachable",
								vim.log.levels.WARN
							)
						end)
					end
